...
export async function verifyEmail(payload) {
  ...
  if (typeof window !== 'undefined' && response) {
    ...
    console.log('[authApi] Extracted from verification:', { hasToken: !!token, hasUser: !!user, userType });
    
    if (token) {
      // Store role-specific token
      if (userType === 'rider' || userType === 'driver') {
        localStorage.setItem('rider_token', token);
        console.log('[authApi] Rider token stored after verification');
      } else {
        localStorage.setItem('customer_token', token);
        console.log('[authApi] Customer token stored after verification');
      }
      localStorage.setItem('auth_token', token);
